...
def draw_investing(chat_id, month_investing=5000, count_years=10, percentage_return=10, seed_money=0):
    data = pd.DataFrame()
    ...
    plt.title(str(count_years) + ' YEARS, ' + str(percentage_return) + ' ANNUAL PERCENT, ' + total, fontsize=20)
...
def start_message(message):
    bot.send_message(message.chat.id, 'Hello, I am investing bot.\nI can calculate investment profit.\n\
...
def send_text(message):
    ...
    if message.text == 'Привет':
        ...
        try:
            ...
            message = draw_investing(chat_id, month_investing, count_years, percentage_return, seed_money)
            time.sleep(1)
